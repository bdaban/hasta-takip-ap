    ...
    <div class="bg-white rounded-lg shadow-md">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h1 class="text-xl font-bold">Randevu Düzenle</h1>
          <NuxtLink
            to="/randevular"
...
const { data: randevuData } = await useFetch<Randevu>(`/api/randevular/${route.params.id}`)
...
watchEffect(() => {
  ...
  if (randevuData.value) {
    form.value = {
      hastaId: randevuData.value.hasta.id,
      doktorId: randevuData.value.doktor.id,
      tarih: randevuData.value.tarih,
      durum: randevuData.value.durum,
      notlar: randevuData.value.notlar || ''
    }
  }
})
...
const handleSubmit = async () => {
  loading.value = true
  try {
    const { error } = await useFetch(`/api/randevular/${route.params.id}`, {
      method: 'PUT',
      body: form.value
    })
    if (error.value) {
