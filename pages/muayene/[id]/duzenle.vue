    ...
    <div class="bg-white rounded-lg shadow-md">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h1 class="text-xl font-bold">Muayene Düzenle</h1>
          <NuxtLink
            :to="`/muayene/${id}`"
...
watchEffect(() => {
  ...
  if (muayene.value) {
    form.value = {
      hastaId: muayene.value.hastaId,
      doktorId: muayene.value.doktorId,
      tarih: muayene.value.tarih,
      sikayet: muayene.value.sikayet,
      tani: muayene.value.tani,
      tedavi: muayene.value.tedavi,
      durum: muayene.value.durum,
      notlar: muayene.value.notlar
    }
  }
})
...
const handleSubmit = async () => {
  loading.value = true
  try {
    const { error } = await useFetch(`/api/muayeneler/${id}`, {
      method: 'PUT',
      body: form.value
    })
    if (error.value) {
    ...
    navigateTo(`/muayene/${id}`)
  } catch (err: any) {
    console.error('Muayene güncellenirken hata oluştu:', err)
  } finally {
    loading.value = false
  }
}
